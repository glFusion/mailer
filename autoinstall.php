...
$INSTALL_plugin['mailer'] = array(
    ...
    'plugin' => array(
        'type'      => 'plugin', 
        'name'      => $_MLR_CONF['pi_name'],
        'ver'       => $_MLR_CONF['pi_version'], 
        'gl_ver'    => $_MLR_CONF['gl_version'],
        'url'       => $_MLR_CONF['pi_url'], 
        'display'   => $_MLR_CONF['pi_display_name'],
    ),
    array(
        'type' => 'table', 
...
{
    global $INSTALL_plugin, $_MLR_CONF;

    COM_errorLog("Attempting to install the {$_MLR_CONF['pi_display_name']} plugin", 1);

    $ret = INSTALLER_install($INSTALL_plugin[$_MLR_CONF['pi_name']]);
    if ($ret > 0) {
        return false;
    }
...
function plugin_load_configuration_mailer()
{
    ...
    require_once MLR_PI_PATH . 'install_defaults.php';
